...
def _number(value: Any, default: float = 0.0) -> float:
    """Convert an API value to a finite float without crashing on bad data."""
    try:
        result = float(value)
...
def _format_number(value: Any, prefix: str = "") -> str:
    """Format large financial values compactly for Telegram."""
...
def _format_price(value: Any) -> str:
    """Format token prices while retaining useful precision for microcaps."""
    number = _number(value)
    if number == 0:
        return "n/a"
    if abs(number) >= 1:
...
def _format_age(pair_created_at: Any) -> str:
    """Return a human-readable age from DEX Screener's millisecond timestamp."""
...
async def _wait_for_dex_request() -> None:
    """Keep requests comfortably below the documented public API limits."""
...
async def _get_dex_json(path: str) -> Any:
    """Fetch and decode a DEX Screener endpoint with bounded retry/backoff."""
    ...
    for attempt in range(MAX_RETRIES):
        ...
        try:
            response = await asyncio.to_thread(
                urlopen, request, timeout=HTTP_TIMEOUT_SECONDS
            )
            with response:
...
def _is_meme_candidate(profile: dict[str, Any]) -> bool:
    """Require an explicit public meme signal; DEX listing alone is not enough."""
    text_parts = [
        str(profile.get(key, "")) for key in ("description", "url", "tokenAddress")
    ]
...
def _analyze_pair(pair: dict[str, Any]) -> dict[str, Any]:
    """Create transparent opportunity, quality, and preliminary risk metadata."""
    ...
    if _number(pair.get("pairCreatedAt")) > 0:
        pair_age_hours = max(
            0.1,
            (time.time() * 1000 - _number(pair.get("pairCreatedAt"))) / 3_600_000,
        )
    ...
    volume_acceleration = (
        volume_h1 / (volume / 24) if volume_h1 and volume > 0 else 0.0
    )
    ...
    required_fields = (
        ...
        ("txns.h24", "24h buys/sells"),
        ("pairCreatedAt", "token age"),
        ("url", "DEX Screener URL"),
    )
    for path, label in required_fields:
        if path == "liquidity.usd":
            present = liquidity > 0
        elif path == "volume.h24":
            present = volume > 0
        elif path == "priceChange.h24":
            present = isinstance(pair.get("priceChange"), dict) and "h24" in pair["priceChange"]
        elif path == "txns.h24":
            present = isinstance(txns_24h, dict) and (
                "buys" in txns_24h or "sells" in txns_24h
            )
        elif path == "pairCreatedAt":
            present = pair_age_hours > 0
        else:
            present = bool(pair.get(path))
        if not present:
            unknown.append(label)

    # These fields are not in the documented pair response and must not be inferred.
    unknown.extend(
        [
            "holder growth",
            "holder concentration",
            "developer/contract risk",
            "liquidity lock",
    ...
    completeness = round(
        100
        * (len(required_fields) - len([item for item in unknown if item not in {
            "holder growth",
            "holder concentration",
            "developer/contract risk",
            "liquidity lock",
    ...
    risk_flags: list[str] = []
    if price_change <= -90:
        anomalies.append("down >90%")
        risk_flags.append("extreme drawdown")
    if price_change >= 200:
        anomalies.append("extreme price increase")
        risk_flags.append("extreme momentum")
    if liquidity and liquidity < 5_000:
        anomalies.append("very low liquidity")
        risk_flags.append("very low liquidity")
    if volume_to_liquidity > 50:
        anomalies.append("extreme volume/liquidity")
        risk_flags.append("volume/liquidity anomaly")
    if volume_to_market_cap > 5:
        anomalies.append("extreme volume/market cap")
        risk_flags.append("volume/market-cap anomaly")
    if "market cap" in unknown or "liquidity" in unknown:
        risk_flags.append("missing critical risk data")
    if str(pair.get("chainId", "")).lower() not in SUPPORTED_CHAINS:
        risk_flags.append("unsupported/unusual chain")
    if "price" in unknown or "24h buys/sells" in unknown:
        risk_flags.append("incomplete activity data")

    # Moderate, multi-factor signals; anomaly conditions are explicitly zeroed or penalized.
    liquidity_score = _relative_score(liquidity, 10_000, 100_000)
    ratio_score = (
        100
        if 5 <= market_cap_to_liquidity <= 50
        else max(0.0, 100 - abs(market_cap_to_liquidity - 25) * 3)
        if market_cap_to_liquidity
        else 0.0
    )
    ...
    age_score = (
        max(0.0, 100 - max(0, pair_age_hours - 24) * 2)
        if pair_age_hours
        else 0.0
    )
    momentum_score = (
        max(0.0, min(100.0, 50 + price_change))
        if -50 < price_change < 100
        else 0.0
    )
    opportunity = (
        liquidity_score * 0.18
        + ratio_score * 0.12
        + relative_volume_score * 0.13
        + volume_cap_score * 0.10
    ...
    if completeness < 55:
        ...
        anomaly in anomalies
        for anomaly in (
            "down >90%",
            "extreme volume/liquidity",
            "extreme volume/market cap",
            "very low liquidity",
        )
    ):
        verdict = "EXTREME RISK"
    elif len(risk_flags) >= 3 or "unsupported/unusual chain" in risk_flags:
        verdict = "HIGH RISK"
    elif price_change >= 50 or volume_acceleration >= 3:
...
def _score_pair(pair: dict[str, Any]) -> float:
    """Sort by the bounded opportunity score, not one market metric."""
    return _number((pair.get("_analysis") or {}).get("opportunity_score"))


def _apply_birdeye_risk_evidence(
    pair: dict[str, Any], evidence: BirdeyeRiskEvidence
) -> None:
    """Merge source-attributed evidence without treating it as rug proof."""
    ...
    if evidence.holder_count is not None:
        unknown[:] = [
            item for item in unknown if item.lower() not in {"holder count"}
        ]
    if evidence.top10_holder_percent is not None:
        unknown[:] = [
            ...
            for item in unknown
            if item.lower()
            not in {"holder concentration", "top-holder concentration"}
        ]

    if evidence.top10_holder_percent is not None:
        if evidence.top10_holder_percent >= 70:
            analysis["opportunity_score"] = max(
                0, _number(analysis.get("opportunity_score")) - 12
            )
            if analysis.get("verdict") not in {
                "EXTREME RISK",
                "INSUFFICIENT DATA",
            }:
            ...
            analysis["opportunity_score"] = max(
                0, _number(analysis.get("opportunity_score")) - 5
            )
    pair["_analysis"] = analysis


def _apply_developer_risk_evidence(
    pair: dict[str, Any], developer: dict[str, Any]
) -> None:
    """Use developer history only as a bounded, corroborating risk signal."""
    analysis = pair.get("_analysis") or {}
    raw_score = developer.get("developer_score", "UNKNOWN")
    analysis["developer_score"] = raw_score
    analysis["developer_risk"] = developer.get("risk", "UNKNOWN")
    risk = str(developer.get("risk") or "").upper()
    try:
        score = float(raw_score)
    except (TypeError, ValueError):
        score = None

    independent_flags = list(analysis.get("risk_flags") or [])
    developer_concern = risk == "HIGH" or (score is not None and score < 40)
    analysis["developer_risk_corroborated"] = bool(
        developer_concern and independent_flags
    )
    if developer_concern and independent_flags:
        risk_flags = analysis.setdefault("risk_flags", [])
        signal = "developer history concern corroborates independent risk evidence"
        if signal not in risk_flags:
            risk_flags.append(signal)
        penalty = 8 if risk == "HIGH" else min(6, max(2, round((40 - score) / 5)))
        analysis["opportunity_score"] = max(
            0, _number(analysis.get("opportunity_score")) - penalty
        )
        analysis["developer_score_penalty"] = penalty
        if analysis.get("verdict") not in {
            "EXTREME RISK",
            "INSUFFICIENT DATA",
            "HIGH RISK",
        ...
        analysis["developer_score_penalty"] = 0
    pair["_analysis"] = analysis
...
async def _scan_newly_active_coins() -> list[dict[str, Any]]:
    """Get recent profiles and enrich them with current pair metrics."""
    profiles_data = await _get_dex_json(LATEST_PROFILES_PATH)
    ...
    meme_profiles = [profile for profile in profiles if _is_meme_candidate(profile)]
    ...
    for chain_id, addresses in grouped.items():
        for offset in range(0, len(addresses), MAX_TOKEN_ADDRESSES_PER_REQUEST):
            batch = addresses[offset : offset + MAX_TOKEN_ADDRESSES_PER_REQUEST]
            path = TOKEN_BATCH_PATH.format(
                chain_id=quote(chain_id, safe=""),
                addresses=quote(",".join(batch), safe=","),
            )
            response = await _get_dex_json(path)
            if isinstance(response, list):
                pairs.extend(pair for pair in response if isinstance(pair, dict))

    # Keep the best pair per token, exclude unsupported chains, and require a
    # recognizable meme signal from public metadata plus pair token metadata.
    ...
    for pair in pairs:
        ...
        has_meme_signal = any(keyword in token_text for keyword in MEME_KEYWORDS)
        if key[0].lower() not in SUPPORTED_CHAINS or not has_meme_signal:
            continue
        analyzed = _analyze_pair(pair)
        if key[0] and key[1] and (
            key not in best_by_token or _score_pair(analyzed) > _score_pair(best_by_token[key])
        ):
            best_by_token[key] = analyzed
    return sorted(best_by_token.values(), key=_score_pair, reverse=True)[:10]


def _render_scan_results(
    pairs: list[dict[str, Any]], include_header: bool = True
) -> str:
    """Render a concise scan card while retaining detailed evidence internally."""
    if not pairs:
        return (
            "No newly active meme coin pairs are available right now.\n"
            "Try /scan again shortly."
        )

    lines = (
        ["🔎 MEMEFINDER 1000X", "Read-only analysis. No trades are executed.", ""]
        if include_header
        else []
    )
    for pair in pairs:
        ...
        top10_icon = (
            "🔴"
            if birdeye_top10 is not None and birdeye_top10 >= 50
            else "🟢"
            if birdeye_top10 is not None
            else ""
        )
        developer_address = str(developer.get("developer") or "UNKNOWN")
        if developer_address.lower() != "unknown" and len(developer_address) > 12:
            developer_address = f"{developer_address[:4]}...{developer_address[-4:]}"
        developer_label = _clip(
            developer.get("developer_label"), 30, "Probable Developer"
        )
        ...
        history_available = str(
            developer.get("previous_launches") or "UNKNOWN"
        ).upper() != "UNKNOWN"
        if history_available:
            developer_lines = [
                (
                    "Launches: "
                    f"{developer.get('previous_launches', 'UNKNOWN')} | "
                    f"Successful: {developer.get('successful_launches', 'UNKNOWN')} | "
                    f"Failed/inactive: {developer.get('failed_launches', 'UNKNOWN')}"
                ),
                (
                    "Suspected rug patterns: "
                    f"{developer.get('suspected_rug_launches', 'UNKNOWN')}"
                ),
                f"Best previous: {_clip(developer.get('best_previous_launch'), 120)}",
                (
                    f"Developer Score: {_clip(developer.get('developer_score'), 80)} | "
                    f"Risk: {_clip(developer.get('risk'), 30)}"
                ),
            ]
            optional_history = (
                ("Historical market cap", "highest_historical_market_cap", 60),
                ("Historical liquidity", "historical_liquidity", 80),
                ("Liquidity activity", "liquidity_activity", 120),
                ("Developer selling", "selling_behavior", 120),
                ("Activity timing", "major_activity_timing", 120),
                ("Recent launches", "recent_launch_activity", 120),
            )
            for label, key, limit in optional_history:
                value = str(developer.get(key) or "UNKNOWN")
                if value.upper() != "UNKNOWN":
                    developer_lines.append(f"{label}: {_clip(value, limit)}")
            developer_lines.extend(
                [
                    f"Coverage: {history_coverage}",
                    f"Evidence: {developer_evidence}",
                ]
            )
        else:
            developer_lines = [
                "History: UNKNOWN — no attributable prior launches in available evidence",
                "Developer Score: UNKNOWN | Risk: UNKNOWN",
                f"Coverage: {history_coverage}",
                f"Evidence: {developer_evidence}",
            ]
        concise_risks = list(
            dict.fromkeys(_clip(item, 140) for item in risk_flags)
        )
        ...
        lines.extend(
            [
                (
                    f"🐸 {_clip(base_token.get('symbol'), 32)} · "
                    f"{_clip(pair.get('chainId'), 20).upper()}"
                ),
                "",
                f"Opportunity: {analysis.get('opportunity_score', 'UNKNOWN')}/100",
                f"Risk: {risk_display}",
                "",
                f"Price: {_format_price(pair.get('priceUsd')).replace('n/a', 'UNKNOWN')}",
                f"Market Cap: {_format_number(pair.get('marketCap'), '$').replace('n/a', 'UNKNOWN')}",
                f"Liquidity: {_format_number(liquidity, '$').replace('n/a', 'UNKNOWN')}",
                f"Volume: {_format_number((pair.get('volume') or {}).get('h24'), '$').replace('n/a', 'UNKNOWN')}",
                f"Age: {_format_age(pair.get('pairCreatedAt')).replace('n/a', 'UNKNOWN')}",
                "",
                "📊 MARKET",
                f"24h: {change_24h:+.2f}% | Age: {_format_age(pair.get('pairCreatedAt')).replace('n/a', 'UNKNOWN')}",
                f"Buys: {int(_number(txns_24h.get('buys')))}",
                f"Sells: {int(_number(txns_24h.get('sells')))}",
                f"Volume anomaly: {'🔴' if volume_anomaly else '🟢'}",
                "",
                "👥 HOLDERS",
                f"Holders: {birdeye_holder_count if birdeye_holder_count is not None else 'UNKNOWN'}",
                f"Top 10: {f'{birdeye_top10:.1f}%' if birdeye_top10 is not None else 'UNKNOWN'} {top10_icon}".rstrip(),
                "",
                "👨‍💻 DEVELOPER",
                f"{developer_label}: {developer_address}",
            ]
        )
        ...
        lines.extend(
            [
                "",
                "🐋 SMART MONEY",
                f"Top traders: {_clip(smart_wallet.get('top_traders_detected'), 30)}",
                f"Qualified wallets: {_clip(smart_wallet.get('qualified_wallet_count'), 30)}",
                f"Smart Money Score: {_clip(smart_wallet.get('smart_money_score'), 50)}",
                "",
                "🚨 RISK",
            ]
        )
...
def _render_scan_messages(pairs: list[dict[str, Any]]) -> list[str]:
    """Paginate complete token cards within Telegram's character limit."""
    if not pairs:
    ...
    for pair in pairs:
        ...
        if len(card) > MAX_TELEGRAM_MESSAGE_LENGTH:
            ...
            card = "\n".join(
                [
                    (
                        f"🐸 {_clip(base.get('symbol'), 32)} · "
                        f"{_clip(pair.get('chainId'), 20).upper()}"
                    ),
                    "Result available, but detailed fields exceeded Telegram limits.",
                    (
                        "VERDICT: "
                        f"{_clip(analysis.get('verdict'), 40, 'INSUFFICIENT DATA')}"
                    ),
                    f"DEX: {_clip(pair.get('url'), 300)}",
                ]
            )
...
async def scan(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Fetch and reply with the current top ten scan candidates."""
    ...
    try:
        ...
        messages = [
            "DEX Screener is temporarily unavailable or rate-limited. "
            "Please try /scan again in a moment."
        ]
    except Exception:
        logger.exception("Unexpected scan error")
        messages = [
            "The scan could not be completed right now. Please try /scan again."
        ]
...
def main() -> None:
    ...
    if not token:
        raise RuntimeError(
            "TELEGRAM_BOT_TOKEN is not set. Add it as an environment variable "
    ...
    # Telegram API URLs contain the bot token; keep dependency request logs
    # above INFO so credentials never appear in workflow output.
