...
WELCOME_MESSAGE = "🚀 MemeFinder 1000X is online! Meme coin scanner coming soon."
...
SUPPORTED_CHAINS = {"arbitrum", "base", "bsc", "ethereum", "optimism", "polygon", "solana", "sui", "avalanche", "linea", "fantom", "cronos", "ton"}
MEME_KEYWORDS = ("meme", "doge", "shib", "pepe", "inu", "cat", "frog", "moon", "wojak", "bonk", "floki")
...
def _format_number(value: Any, prefix: str = "") -> str:
    ...
    absolute = abs(number)
    if absolute >= 1_000_000_000: return f"{prefix}{number / 1_000_000_000:.2f}B"
    if absolute >= 1_000_000: return f"{prefix}{number / 1_000_000:.2f}M"
    if absolute >= 1_000: return f"{prefix}{number / 1_000:.2f}K"
    return f"{prefix}{number:.2f}"

def _format_price(value: Any) -> str:
    number = _number(value)
    if number == 0: return "n/a"
    if abs(number) >= 1: return f"${number:,.4f}"
    return f"${number:.8f}".rstrip("0").rstrip(".")

def _format_age(pair_created_at: Any) -> str:
    created_ms = _number(pair_created_at)
    if created_ms <= 0: return "n/a"
    age_seconds = max(0, time.time() - created_ms / 1000)
    if age_seconds < 3600: return f"{max(1, int(age_seconds // 60))}m"
    if age_seconds < 86_400: return f"{int(age_seconds // 3600)}h"
...
async def _get_dex_json(path: str) -> Any:
    ...
    for attempt in range(MAX_RETRIES):
        ...
        try:
            ...
            last_error = error
            if error.code not in (429, 500, 502, 503, 504): raise DexScreenerError(f"DEX Screener returned HTTP {error.code}") from error
...
def _is_meme_candidate(profile: dict[str, Any]) -> bool:
    ...
    for link in profile.get("links") or []:
        if isinstance(link, dict): text_parts.extend(str(link.get(key, "")) for key in ("label", "url"))
...
def _relative_score(value: float, low: float, high: float) -> float:
    ...
    if value <= low: return value / low * 50
    if value <= high: return 50 + (value - low) / (high - low) * 50
    return 100.0
...
def _analyze_pair(pair: dict[str, Any]) -> dict[str, Any]:
    ...
    pair_age_hours = 0.0
    if _number(pair.get("pairCreatedAt")) > 0: pair_age_hours = max(0.1, (time.time() * 1000 - _number(pair.get("pairCreatedAt"))) / 3_600_000)
    ...
    unknown: list[str] = []
    required_fields = (("priceUsd", "price"), ("marketCap", "market cap"), ("liquidity.usd", "liquidity"), ("volume.h24", "24h volume"), ("priceChange.h24", "24h price change"), ("txns.h24", "24h buys/sells"), ("pairCreatedAt", "token age"), ("url", "DEX Screener URL"))
...
def _render_scan_results(pairs: list[dict[str, Any]], include_header: bool = True) -> str:
    ...
    for pair in pairs:
        ...
        if history_available:
            developer_lines = [f"Launches: {developer.get('previous_launches', 'UNKNOWN')} | Successful: {developer.get('successful_launches', 'UNKNOWN')} | Failed/inactive: {developer.get('failed_launches', 'UNKNOWN')}", f"Suspected rug patterns: {developer.get('suspected_rug_launches', 'UNKNOWN')}", f"Best previous: {_clip(developer.get('best_previous_launch'), 120)}", f"Developer Score: {_clip(developer.get('developer_score'), 80)} | Risk: {_clip(developer.get('risk'), 30)}"]
        ...
        lines.extend([f"🐸 {_clip(base_token.get('symbol'), 32)} · {_clip(pair.get('chainId'), 20).upper()}", "", f"Opportunity: {analysis.get('opportunity_score', 'UNKNOWN')}/100", f"Risk: {risk_display}", "", f"Price: {_format_price(pair.get('priceUsd')).replace('n/a', 'UNKNOWN')}", f"Market Cap: {_format_number(pair.get('marketCap'), '$').replace('n/a', 'UNKNOWN')}", f"Liquidity: {_format_number(liquidity, '$').replace('n/a', 'UNKNOWN')}", f"Volume: {_format_number((pair.get('volume') or {}).get('h24'), '$').replace('n/a', 'UNKNOWN')}", f"Age: {_format_age(pair.get('pairCreatedAt')).replace('n/a', 'UNKNOWN')}", "", "📊 MARKET", f"24h: {change_24h:+.2f}% | Age: {_format_age(pair.get('pairCreatedAt')).replace('n/a', 'UNKNOWN')}", f"Buys: {int(_number(txns_24h.get('buys')))}", f"Sells: {int(_number(txns_24h.get('sells')))}", f"Volume anomaly: {'🔴' if volume_anomaly else '🟢'}", "", "👥 HOLDERS", f"Holders: {birdeye_holder_count if birdeye_holder_count is not None else 'UNKNOWN'}", f"Top 10: {f'{birdeye_top10:.1f}%' if birdeye_top10 is not None else 'UNKNOWN'} {top10_icon}".rstrip(), "", "👨‍💻 DEVELOPER", f"{developer_label}: {developer_address}"])
        lines.extend(developer_lines)
        if str(pair.get("chainId") or "").lower() == "solana": lines.extend(["", "🔐 SECURITY", f"Score: {security_score_text}", f"Risk: {security_risk} | Confidence: {security_confidence}", f"Mint authority: {security.get('mint_authority', 'UNKNOWN')}", f"Freeze authority: {security.get('freeze_authority', 'UNKNOWN')}", f"Honeypot: {security.get('honeypot', 'UNKNOWN')}", f"LP status: {security.get('lp_status', 'UNKNOWN')}", f"LP lock/burn: {security.get('lp_lock_burn', 'UNKNOWN')}", f"Top 10: {_security_percent(security.get('top10_percent'))}", f"Dev: {_security_percent(security.get('dev_percent'))}", f"Insiders: {_security_percent(security.get('insider_percent'))}", f"Snipers: {_security_percent(security.get('sniper_percent'))}", f"Bundlers: {_security_percent(security.get('bundler_percent'))}"])
...
def _render_scan_messages(pairs: list[dict[str, Any]]) -> list[str]:
    ...
    for pair in pairs:
        ...
        if len(card) > MAX_TELEGRAM_MESSAGE_LENGTH:
            base = pair.get("baseToken") or {}; analysis = pair.get("_analysis") or {}
    ...
    messages: list[str] = []; current = header
    for card in cards:
...
async def scan(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        for pair in pairs:
            base_token = pair.get("baseToken") or {}; key = (str(pair.get("chainId") or ""), str(base_token.get("address") or "")); finding = findings.get(key); pair["_developer"] = finding.__dict__ if finding else {}
        smart_findings = await _smart_wallet_intelligence.analyze_many(pairs)
        for index, pair in enumerate(pairs):
            base_token = pair.get("baseToken") or {}; key = (str(pair.get("chainId") or ""), str(base_token.get("address") or "")); finding = smart_findings.get(key); pair["_smart_wallet"] = finding.__dict__ if finding else {}
            if index < 3 and key[0].lower() == "solana" and key[1]:
                snapshot = await _birdeye_provider.analyze_token(key[1]); _apply_birdeye_risk_evidence(pair, _birdeye_provider.risk_evidence(snapshot))
            _apply_developer_risk_evidence(pair, pair.get("_developer") or {})
        for pair in pairs:
            base_token = pair.get("baseToken") or {}; chain = str(pair.get("chainId") or "").lower(); address = str(base_token.get("address") or "")
            if chain != "solana" or not address: continue
            try:
                top10_hint = (pair.get("_analysis") or {}).get("birdeye_top10_holder_percent")
                security = await _security_intelligence.analyze(_birdeye_provider, address, top10_hint=top10_hint)
                pair["_security"] = security.__dict__
            except Exception:
                logger.exception("Security analysis failed for token"); pair["_security"] = {"risk": "INCONCLUSIVE", "confidence": "LOW"}
        messages = _render_scan_messages(pairs)
    except DexScreenerError as error:
        logger.warning("DEX Screener scan failed: %s", error); messages = ["DEX Screener is temporarily unavailable or rate-limited. Please try /scan again in a moment."]
    except Exception:
        logger.exception("Unexpected scan error"); messages = ["The scan could not be completed right now. Please try /scan again."]
...
def main() -> None:
    ...
    if not token: raise RuntimeError("TELEGRAM_BOT_TOKEN is not set. Add it as an environment variable before starting the bot.")
    logging.basicConfig(format="%(asctime)s - %(name)s - %(levelname)s - %(message)s", level=logging.INFO); logging.getLogger("httpx").setLevel(logging.WARNING)
    application = Application.builder().token(token).build(); application.add_handler(CommandHandler("start", start)); application.add_handler(CommandHandler("scan", scan))
    webhook_url = os.getenv("TELEGRAM_WEBHOOK_URL")
    if not webhook_url: raise RuntimeError("TELEGRAM_WEBHOOK_URL is not set. Configure the public Railway URL before starting the bot.")
    port = int(os.getenv("PORT", "8080")); application.run_webhook(listen="0.0.0.0", port=port, webhook_url=webhook_url, drop_pending_updates=True)
